# Shamelessly taken from  : https://github.com/Broham/suncalcPy
...
def getMoonIllumination(date):
    ...
    return dict(
        ...
        moonphase= 0.5 + 0.5 * inc * (-1 if angle < 0 else 1) / PI,
        moonangle= angle
    )
...
def getMoonTimes(date, lat, lng):
    ...
    t = datetime.combine(date, datetime.min.time())
    ...
    if (rise):
        result["moonrise"] = hoursLater(t, rise).strftime('%Y-%m-%d %H:%M:%S')
    if (sett):
        result["moonset"] = hoursLater(t, sett).strftime('%Y-%m-%d %H:%M:%S')
